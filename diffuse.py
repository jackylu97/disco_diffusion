                    ...
                    for k, image in enumerate(sample["pred_xstart"]):
                        ...
                        if args.n_batches > 0:
                            ...
                            if cur_t == -1 and args.intermediates_in_subfolder is True:
                                save_num = (
                                    f"{frame_num:04}" if animation_mode != "None" else i
                                )
                                filename = (
                                    f"{args.batch_name}({args.batchNum})_{save_num}.png"
                                )
                            else:
...
def save_settings():
    ...
    with open(
        f"{fpm.batchFolder}/{batch_name}({batchNum})_settings.txt", "w+"
    ) as f:  # save settings
...
# DEFINE SECONDARY DIFFUSION MODEL
...
text_prompts = {
    0: [
        ...
        "oil painting by james gurney:3",
        "a scene depicting an ancient king:3",
...
display_rate = 30  # @param{type: 'number'}
...
if resume_run:
    ...
    if resume_from_frame == "latest":
        ...
        if retain_overwritten_frames is True:
            existing_frames = len(
                glob(fpm.batchFolder + f"/{batch_name}({batchNum})_*.png")
            )
            frames_to_save = existing_frames - start_frame
            print(f"Moving {frames_to_save} frames to the Retained folder")
            fpm.move_files(
                batchNum, start_frame, existing_frames, fpm.batchFolder, retainFolder
            )
else:
    start_frame = 0
    batchNum = len(glob(fpm.batchFolder + "/*.txt"))
    while (
        path.isfile(f"{fpm.batchFolder}/{batch_name}({batchNum})_settings.txt") is True
        or path.isfile(f"{fpm.batchFolder}/{batch_name}-{batchNum}_settings.txt")
        is True
    ):
        batchNum += 1
